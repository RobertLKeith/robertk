...
<Seo {seo} {profile}/>{#if loading}<Loader on:complete={()=>loading=false}/>{/if}<Hero {profile} {agency} {heroImage} {resumeUrl} hasShowreels={availableReels.length > 0}/><main>
...
<section id="credits" class="section"><div class="container"><header class="section-head" data-reveal><div><p class="eyebrow">Résumé</p><h2>Credits</h2></div></header><Credits {credits}/><div class="link-row" data-reveal><a href={resumeUrl} download="Robert Keith EA Resume.doc">Download full resume ↓</a><a href={profile.imdbUrl} target="_blank" rel="noopener noreferrer">IMDb ↗</a><a href={profile.actorsAccessUrl} target="_blank" rel="noopener noreferrer">Actors Access ↗</a></div></div></section>
